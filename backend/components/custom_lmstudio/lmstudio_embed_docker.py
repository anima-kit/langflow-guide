...
class LMStudioEmbeddingswithDockerComponent(LCEmbeddingsModel):
    display_name: str = "LM Studio Embeddings with Docker"
    description: str = "Generate embeddings using local models LM Studio through Docker."
    icon = "LMStudio"
    name = "lmstudio_embed_docker"
    ...
    async def update_build_config(self, build_config: dict, field_value: Any, field_name: str | None = None):  # noqa: ARG002
        if field_name == "model":
            ...
            if base_url_load_from_db:
                ...
                base_url_value = "http://host.docker.internal:1234/v1"
